...
import {
  ...
  Check, ChevronDown, ChevronUp, Send, X,
} from "lucide-react";
...
function RunDialog({ events, onSend, onClose }: {
  events: TennisEvent[];
  onSend: (indices: number[]) => void;
  onClose: () => void;
}) {
  const [checked, setChecked] = useState<Set<number>>(new Set(events.map((_, i) => i)));
  ...
  return (
    <Dialog.Portal>
      ...
      <Dialog.Content className="fixed z-50 top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-white rounded-2xl shadow-2xl w-[480px] max-h-[70vh] flex flex-col border border-[#C5DDB8]">
        <div className="px-6 py-4 border-b border-[#E0EDD8] flex items-center justify-between">
          <div>
            <Dialog.Title className="text-base font-bold text-[#1A2B1A]">Select events to send</Dialog.Title>
            <Dialog.Description className="text-xs text-[#5C7A5C] mt-0.5">Choose which upcoming practices to include in the message</Dialog.Description>
          </div>
          <button onClick={onClose} className="text-[#5C7A5C] hover:text-[#1A2B1A] p-1 rounded-lg hover:bg-[#F0F7EC]"><X size={16} /></button>
        </div>
        <ScrollArea.Root className="flex-1 overflow-hidden">
          <ScrollArea.Viewport className="p-4 space-y-2">
            {events.length === 0 ? (
              <p className="text-sm text-[#5C7A5C] text-center py-8">No upcoming events found for the configured lookhead period.</p>
            ) : events.map((event, i) => (
              <label key={i} className={`flex items-start gap-3 p-3 rounded-xl border-2 cursor-pointer transition-all ${checked.has(i) ? "border-[#1B6B2C] bg-[#F0F7EC]" : "border-[#E0EDD8] hover:border-[#A8C99A]"}`}>
                <Checkbox.Root
                  checked={checked.has(i)}
                  onCheckedChange={() => toggle(i)}
                  className="w-5 h-5 rounded border-2 border-[#1B6B2C] bg-white flex-shrink-0 mt-0.5 flex items-center justify-center data-[state=checked]:bg-[#1B6B2C]"
                >
                  <Checkbox.Indicator><Check size={12} className="text-white" /></Checkbox.Indicator>
                </Checkbox.Root>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between gap-2">
                    <span className="font-semibold text-sm text-[#1A2B1A] truncate">{event.title}</span>
                    <WeatherBadge w={event.weather} />
                  </div>
                  <div className="flex items-center gap-3 mt-1 text-xs text-[#5C7A5C]">
                    <span className="flex items-center gap-1"><Calendar size={10} />{fmtDate(event.startTime)}</span>
                    <span className="flex items-center gap-1"><Clock size={10} />{fmtTime(event.startTime)}–{fmtTime(event.endTime)}</span>
                  </div>
                  <div className="flex items-center gap-1 mt-0.5 text-xs text-[#5C7A5C]">
        ...
        <div className="px-6 py-4 border-t border-[#E0EDD8] flex items-center justify-between">
          <span className="text-xs text-[#5C7A5C]">{checked.size} of {events.length} selected</span>
          <div className="flex gap-2">
            <button onClick={onClose} className="px-4 py-2 rounded-lg border border-[#C5DDB8] text-sm text-[#5C7A5C] hover:bg-[#F0F7EC] transition-colors">Cancel</button>
            <button
              onClick={() => { onSend(Array.from(checked)); onClose(); }}
              disabled={checked.size === 0}
              className="flex items-center gap-2 px-4 py-2 rounded-lg bg-[#1B6B2C] text-white text-sm font-bold hover:bg-[#155523] disabled:opacity-50 transition-colors"
            >
              <Send size={13} /> Send {checked.size > 0 ? `(${checked.size})` : ""}
            </button>
          </div>
        </div>
      </Dialog.Content>
...
export default function Dashboard() {
  ...
  const [runEvents, setRunEvents] = useState<TennisEvent[]>([]);
  ...
  const loadEvents = useCallback(async (days?: number) => {
    ...
    try {
      ...
      setEvents(data);
      // Clear selected if it was deleted from calendar
      setSelected(prev => {
        if (!prev) return null;
  ...
  // Re-fetch when dashboardDays changes
  ...
  const openRunDialog = async () => {
    ...
    try {
      const res = await fetch(`/api/events?days=${config.messageDaysForward}`);
      const data: TennisEvent[] = await res.json();
      setRunEvents(data);
    } catch { showToast("Failed to load events", false); setRunDialogOpen(false); }
    finally { setLoadingRunEvents(false); }
  };

  const doRun = async (selectedIndices: number[]) => {
    setRunning(true);
    try {
      ...
      <Dialog.Root open={runDialogOpen} onOpenChange={setRunDialogOpen}>
                ...
                <ScrollArea.Viewport className="h-full w-full">
                  {loading ? (
                    ...
                    <div className="p-3 space-y-2">
                      {events.map((event, i) => (
                        <button key={i} onClick={() => setSelected(event)}
                      ...
                      <ScrollArea.Viewport className="h-full p-3">
                        {logs.length === 0 ? (
                          <div className="flex flex-col items-center justify-center h-36 text-[#5C7A5C] text-xs gap-2">
                            ...
                            No messages logged yet. Hit "Run Now" to send your first one.
                          </div>
                        ) : (
        ...
        {runDialogOpen && (
          loadingRunEvents ? (
            <Dialog.Portal>
              <Dialog.Overlay className="fixed inset-0 bg-black/40 z-40" />
              <Dialog.Content className="fixed z-50 top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-white rounded-2xl shadow-2xl p-8 flex items-center gap-3">
                <RefreshCw size={18} className="animate-spin text-[#1B6B2C]" />
                <span className="text-sm font-medium text-[#1A2B1A]">Loading events...</span>
              </Dialog.Content>
            </Dialog.Portal>
          ) : (
            <RunDialog events={runEvents} onSend={doRun} onClose={() => setRunDialogOpen(false)} />
          )
        )}
